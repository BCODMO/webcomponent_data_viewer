...
export class BcodmoDataViewer extends LitElement {
  ...
  _setdpkg(){
    if(this.dataset && this.version){
      this.dpkg = 'https://raw.githubusercontent.com/BCODMO/Frictionless-Data-Package-Inspector/ag-grid/datasets/' + this.dataset + '/' + this.version + '/data/datapackage.json';
    }
  }
  ...
  render() {
    ...
    return html`
      <mwc-snackbar id="toast" .labelText=${this._errorText} leading timeoutMs=-1 closeOnEscape>
  ...
  get _getPageContent(){
    ...
    return html`
      ...
      <div style="min-height: 400px;" id=${dataGridId} class="ag-theme-balham"></div>
    `;
  }
  ...
  _start(){
    ...
    datapackage.Package.load(this.dpkg).then((pkg) => {
