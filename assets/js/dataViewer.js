...
const main = async () => {
  ...
  const package = await datapackage.Package.load(dpkg_url).catch((err) => {
    showError(err);
    throw new Error(err);
  });
  ...
  for (const [idx, resource] of package.resources.entries()) {
    if (resource.tabular) {
      if ("undefined" != resource.descriptor.bytes) {
        document.querySelector("#progress-total").innerHTML = prettySize(
          resource.descriptor.bytes
        );
...
async function createDataGrid(idx, resource) {
  ...
  const stream = await resource.iter({
    stream: true,
    keyed: true,
    cast: false,
  });
